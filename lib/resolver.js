...
const getAttributes = object => Object.keys(object).filter(key => Object.keys(object[key]).length === 0);
...
function resolverFactory(targetMaybeThunk) {
  ...
  return (() => {
    var _ref = (0, _bluebird.coroutine)(function* (source, args, context, info) {
      ...
      const fields = (0, _graphqlFields2.default)(info);
      const references = Object.keys(model.rawAttributes).filter(function (key) {
        return model.rawAttributes[key].references || model.rawAttributes[key].primaryKey;
      });
      const exclude = ['__typename'];
      let targetAttributes = references.concat(getAttributes(fields)).filter(function (field) {
        return !exclude.includes(field);
      });
      let findOptions = (0, _argsToFindOptions2.default)(args, targetAttributes);
      ...
      findOptions.attributes = _lodash2.default.uniq(targetAttributes);
